<template>
  <v-container class="mt-5" fluid grid-list-md>
    <v-layout v-if="loadedDays[0]" row wrap>
      ...
      <v-flex xs12>
        <div v-on:click="updateVisibleDays" v-if="loadedDays.length > 5 " class="text-xs-center">
          <v-pagination v-model="currentPage" :length="Math.ceil(loadedDays.length / 5)"></v-pagination>
        </div>
      </v-flex>
    </v-layout>
    ...
    <v-layout class="mb-5" v-else row wrap>
      <v-flex v-for="visibleDay in visibleDays" :key="visibleDay.id" xs12 sm4 md3 lg2 row wrap>
        <v-card>
          <v-card-title>
            <h4 class="primary--text">{{visibleDay.date}}</h4>
          </v-card-title>
          <v-divider></v-divider>
          <v-list dense>
            <v-list-tile>
