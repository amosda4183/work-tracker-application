        ...
        <form @submit.prevent = "onAddNewDay">
					<v-layout row wrap>
							<v-flex xs12 sm6 offset-sm3>
									<h1 class="text-xs-center primary--text pt-5 pb-3">
										Today's Information </h1>
							</v-flex>
					</v-layout>
...
export default {
    ...
    computed: {
			...
			timeWorked () { 
				...
				return Math.round(((end.getHours() - start.getHours()) + (end.getMinutes() - start.getMinutes()) / 60) * 100) / 100
			},
			payPerStop () {
				let payPerStop = 0

			if(!this.loadedDays[0]){
				payPerStop = this.payRate

			} else {
				payPerStop = this.loadedDays[0].payRate
			}
				return payPerStop
			},
			...
			payPerHour () { //Calculate hourly rate based on time worked and stops completed
			return Math.round(this.payPerStop * this.totalStops /  this.timeWorked * 100) / 100

			},

			totalPay() { 
				return Math.round((this.payPerStop * this.totalStops) * 100) /100 
			},
